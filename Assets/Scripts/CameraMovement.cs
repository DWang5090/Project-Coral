...
public class CameraMovement : MonoBehaviour {
	...
	//移動
	private float walkSpeed = 5.0f; //移動速度係数
	private float gravity = 0.01f;//重力加速度
	private Vector3 velocity;//現在の移動速度


	//回転
	private float RotSpeed = 100.0f; //回転速度係数
	...
	void PlayerRotation(){
		...
		float NextRotY = transform.eulerAngles.y + RotY * RotSpeed * Time.deltaTime;

		transform.rotation = Quaternion.Euler( 
			transform.eulerAngles.x + RotX * RotSpeed * Time.deltaTime, 
			NextRotY,
			transform.eulerAngles.z
		) ;
	}
